...
macro (add_module_check directory commit abs)
  get_filename_component(dotgit ".git" ABSOLUTE
	BASE_DIR "${abs}")
  file(TIMESTAMP "${dotgit}" timestamp)
...
function (add_module_git timestamp directory abs listfile RESULT commit)
...
function (add_module directory)
  ...
  get_filename_component(listfile "CMakeLists.txt" ABSOLUTE
	BASE_DIR "${abs}")
  add_module_check("${directory}" "${commit}" "${abs}")
  ...
  if(A_GIT)
	add_module_git("${timestamp}" "${directory}" "${abs}" "${listfile}" RESULT ${A_GIT})
	if(RESULT)
	  add_module_check("${directory}" "${commit}" "${abs}")
	  if(timestamp)
		return()
